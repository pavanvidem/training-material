...
Sequence quality control is therefore an essential first step in your analysis. We will use similar tools as described in the ["Quality control" tutorial]({% link topics/sequence-analysis/tutorials/quality-control/tutorial.md %}):
- [Falco](https://falco.readthedocs.io/en/latest/), which is an efficiency-optimized rewrite of [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/), to create a report of sequence quality
- [MultiQC](https://multiqc.info/) ({% cite ewels2016multiqc %}) to aggregate generated reports
- [Cutadapt](https://cutadapt.readthedocs.io/en/stable/guide.html) ({% cite marcel2011cutadapt %}) to improve the quality of sequences via trimming and filtering.
...
>
> 3. Inspect the webpage output of **Falco** {% icon tool %} for the `GSM461177_untreat_paired` sample (forward and reverse)
>
...
>    {: .question}
>
>    As it is tedious to inspect all these reports individually we will combine them with {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %}.
>
> 4. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the Falco reports with the following parameters:
>    - In *"Results"*:
>        - *"Results"*
...
>                        - {% icon param-collection %} *"FastQC output"*: `Falco on collection N: RawData` (output of **Falco** {% icon tool %})
>
...
>
> 1. {% tool [Cutadapt](toolshed.g2.bx.psu.edu/repos/lparsons/cutadapt/cutadapt/5.2+galaxy0) %} with the following parameters to trim low quality sequences:
...
>
> 2. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the Cutadapt reports with the following parameters:
>    - In *"Results"*:
...
>    2. Verify that the datatype is `gtf` (or `gtf.gz`) and not `gff`.
>
...
>    {: .comment}
>
> 2. {% tool [RNA STAR](toolshed.g2.bx.psu.edu/repos/iuc/rgrnastar/rna_star/2.7.11b+galaxy0) %} with the following parameters to map your reads on the reference genome:
...
>
> 3. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the STAR logs with the following parameters:
>    - In *"Results"*:
>        - *"Results"*
...
>
> 2. Inspect the result by clicking the eye-icon.
>
>    > <question-title></question-title>
>    >
...
> #### Duplicate reads
>
> In the Falco/MultiQC report, we saw that some reads are duplicated:
>
> ![Sequence Duplication Levels](../../images/ref-based/fastqc_sequence_duplication_levels_plot.png "Sequence Duplication Levels")
>
...
> >
> > 2. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the MarkDuplicates logs with the following parameters:
> >    - In *"Results"*:
> >        - *"Results"*
...
> >                        - {% icon param-collection %} *"Picard output"*: `MarkDuplicates on collection N: tabular` (output of **MarkDuplicates** {% icon tool %})
> >
> >    > <question-title></question-title>
> >    >
...
> >
> > 1. {% tool [Samtools idxstats](toolshed.g2.bx.psu.edu/repos/devteam/samtools_idxstats/samtools_idxstats/2.0.7) %} with the following parameters:
> >    - {% icon param-collection %} *"BAM file"*: `RNA STAR on collection N: mapped.bam` (output of **RNA STAR** {% icon tool %})
> >
> > 2. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the idxstats logs with the following parameters:
> >    - In *"Results"*:
> >        - *"Results"*
...
> >
> > 3. {% tool [Gene Body Coverage (BAM)](toolshed.g2.bx.psu.edu/repos/nilesh/rseqc/rseqc_geneBody_coverage/5.0.3+galaxy0) %} with the following parameters:
...
> >
> > 4. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the RSeQC results with the following parameters:
> >    - In *"Results"*:
> >        - *"Results"*
...
> >                            - {% icon param-collection %} *"RSeQC gene_body_coverage output"*: `Gene Body Coverage (BAM) on collection N: stats (TXT)` (output of **Gene Body Coverage (BAM)** {% icon tool %})
> >
> >    > <question-title></question-title>
> >    >
...
> > <hands-on-title>Check the number of reads mapped to each chromosome</hands-on-title>
> >
> > 1. {% tool [Read Distribution](toolshed.g2.bx.psu.edu/repos/nilesh/rseqc/rseqc_read_distribution/5.0.3+galaxy0) %} with the following parameters:
...
> >
> > 2. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the Read Distribution results with the following parameters:
> >    - In *"Results"*:
> >        - *"Results"*
> >            - *"Which tool was used generate logs?"*: `RSeQC`
> >                - In *"RSeQC output"*:
...
> >                            - {% icon param-collection %} *"RSeQC read_distribution output"*: `Read Distribution on collection N: stats (TXT)` (output of **Read Distribution** {% icon tool %})
> >
> >    > <question-title></question-title>
> >    >
...
1. We can do a visual inspection of read strands on IGV (for Paired-end dataset it is less easy than with single read and when you have a lot of samples, this can be painful).
    ...
    >    2. Right click and then select `Color Aligments by` -> `first-of-pair strand`
    ...
    >
    > 1. Are the reads evenly distributed between the 2 colors (red - forward and blue - reverse)?
    > 2. What is the type of library strandness?
    >
    > > <solution-title></solution-title>
    > >
    > > 1. Yes we see the same number of reads of both colors.
    ...
    > > >
    > > > Note that all reads are blue for the reverse strand-specific.
    > > {: .comment}
    > {: .solution}
    {: .question}


2. We can do a the same visual inspection of read strands on JBrowse2.

    > <hands-on-title>Estimate strandness with JBrowse2 for a paired-end library</hands-on-title>
    >
    > 1. Go back to your JBrowse2 result (click on the eye icon).
    >
    > 2. Zoom to `chr3R:9445000..9448000` (Chromosome 3 between 9,445 kb to 9,448 kb)
    > 3. Click on the 3 dots close to the name of the bam file (for example GSM461177_untreat_paired) then select `Pileup settings` -> `Color by...` -> `First-of-pair strand`
    > 3. Click on the same 3 dots then select `Pileup settings` -> `Set feature height...` -> `Compact`
    > 4. Repeat with the second bam file.
    >
    {: .hands_on}

    > <question-title></question-title>
    >
    > ![Screenshot of the JBrowse2 view on ps](../../images/ref-based/group_strand_jbrowse2_screenshot.png "Screenshot of JBrowse2 on ps")
    >
    > 1. Are the reads evenly distributed between the 2 colors (red - forward and blue - reverse)?
    > 2. What is the type of library strandness?
    >
    ...
    > > > <comment-title>How would it be if the library was stranded?</comment-title>
    > > >
    > > > ![Screenshot of the JBrowse2 for stranded vs non-stranded](../../images/ref-based/group_strand_jbrowse2_screenshot_RSvsUS.png "Screenshot of JBrowse2 for reverse strand-specific (top) vs. non-stranded (bottom)")
    > > >
    > > > Note that all reads are blue for the reverse strand-specific.
    > > {: .comment}
    > {: .solution}
...
3. Alternatively, instead of using the BAM you can use the stranded coverage generated by **STAR**. Using **pyGenomeTracks** we will be able to visualize the coverage on each strand for each sample. This tool has a lot of parameters to customize your plots.

    > <hands-on-title>Estimate strandness with pyGenometracks from STAR coverage</hands-on-title>
    >
    > 1. {% tool [pyGenomeTracks](toolshed.g2.bx.psu.edu/repos/iuc/pygenometracks/pygenomeTracks/3.9+galaxy0) %}:
    ...
    >                    - {% icon param-file %} *"Track file(s) gtf format"*: `Drosophila_melanogaster.BDGP6.32.109_UCSC.gtf.gz`
    >                - *"height"*: `5`
    {: .hands_on}
...
5. Another option is to estimate these parameters with a tool called **Infer Experiment** from the RSeQC ({% cite wang2012rseqc %}) tool suite.
...
>
> 1. {% tool [featureCounts](toolshed.g2.bx.psu.edu/repos/iuc/featurecounts/featurecounts/2.1.1+galaxy0) %} with the following parameters to count the number of reads per gene:
...
>
> 2. {% tool [MultiQC](toolshed.g2.bx.psu.edu/repos/iuc/multiqc/multiqc/1.27+galaxy4) %} to aggregate the reports with the following parameters:
>    - In *"Results"*:
>        - *"Results"*
